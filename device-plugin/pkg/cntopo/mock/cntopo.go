// Code generated by MockGen. DO NOT EDIT.
// Source: github.com/Cambricon/cambricon-k8s-device-plugin/device-plugin/pkg/cntopo (interfaces: Cntopo)
//
// Generated by this command:
//
//	mockgen -package mock -destination pkg/cntopo/mock/cntopo.go -mock_names=Cntopo=Cntopo github.com/Cambricon/cambricon-k8s-device-plugin/device-plugin/pkg/cntopo Cntopo
//

// Package mock is a generated GoMock package.
package mock

import (
	reflect "reflect"

	cntopo "github.com/Cambricon/cambricon-k8s-device-plugin/device-plugin/pkg/cntopo"
	gomock "go.uber.org/mock/gomock"
)

// Cntopo is a mock of Cntopo interface.
type Cntopo struct {
	ctrl     *gomock.Controller
	recorder *CntopoMockRecorder
	isgomock struct{}
}

// CntopoMockRecorder is the mock recorder for Cntopo.
type CntopoMockRecorder struct {
	mock *Cntopo
}

// NewCntopo creates a new mock instance.
func NewCntopo(ctrl *gomock.Controller) *Cntopo {
	mock := &Cntopo{ctrl: ctrl}
	mock.recorder = &CntopoMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *Cntopo) EXPECT() *CntopoMockRecorder {
	return m.recorder
}

// GetRings mocks base method.
func (m *Cntopo) GetRings(available []uint, size int) ([]cntopo.Ring, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetRings", available, size)
	ret0, _ := ret[0].([]cntopo.Ring)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// GetRings indicates an expected call of GetRings.
func (mr *CntopoMockRecorder) GetRings(available, size any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "GetRings", reflect.TypeOf((*Cntopo)(nil).GetRings), available, size)
}
